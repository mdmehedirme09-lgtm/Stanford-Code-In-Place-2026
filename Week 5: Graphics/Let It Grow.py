...
def grow_leaves(canvas, leaves):
    # 1. Store the inforleavesmation about the shape from our canvas
    left_x, top_y, right_x, bottom_y = get_corner_coordinates(canvas, leaves)
    
    # 2. Delete the original shape from the canvas
    canvas.delete(leaves)
    
    # 3. Remake the object with the transformations (growing it)
    new_leaf = canvas.create_oval(
        left_x - GROWTH_FACTOR,
        top_y - GROWTH_FACTOR,
        right_x + GROWTH_FACTOR,
        bottom_y + GROWTH_FACTOR,
        color="green"
    )
