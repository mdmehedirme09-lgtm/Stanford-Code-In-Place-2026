...
def get_door_clicked(canvas):
    ...
    if y < CANVAS_HEIGHT - DOOR_HEIGHT:
        return None

    door1_x = DOOR_GAP
    door2_x = DOOR_WIDTH + 2 * DOOR_GAP
    door3_x = DOOR_WIDTH * 2 + 3 * DOOR_GAP

    objects = canvas.find_overlapping(x, y, x, y)

    if door1_x <= x <= door1_x + DOOR_WIDTH:
        return objects[-1]
    elif door2_x <= x <= door2_x + DOOR_WIDTH:
        return objects[-1]
    elif door3_x <= x <= door3_x + DOOR_WIDTH:
        return objects[-1]
